...
async function saveNoteEdit(clientId, sessionDate, noteId, journalId, sourceType, sessionId) {
    ...
    try {
        let response;
        console.log('Zapisywanie notatki:', { clientId, sessionDate, noteId, journalId, sourceType, sessionId, newContent });
        ...
        if (sourceType === 'journal') {
            ...
            // Dla sesji indywidualnych - spróbuj różnych endpointów
            if (sessionId && sessionId !== 'null') {
                // Próbuj zapisać przez endpoint sesji
                response = await fetch(`${API}/api/sessions/${sessionId}/notes`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        notes: newContent
                ...
                // Próbuj zaktualizować istniejącą notatkę
            ...
            throw new Error('Nieznany typ źródła: ' + sourceType);
        }
        ...
        console.log('Zapisano pomyślnie:', result);
        ...
        if (clientSelector && clientSelector.value) {
            // Wywołaj funkcję loadClientHistory z głównego scope
            const loadHistoryEvent = new Event('refreshHistory');
            document.dispatchEvent(loadHistoryEvent);
        }
...
// Dodaj event listener dla odświeżania historii
document.addEventListener('refreshHistory', function() {
    const clientSelector = document.getElementById('clientSelector');
    if (clientSelector && clientSelector.value) {
        // Ponowne załadowanie historii
        const loadClientHistory = () => {
            ...
            fetch(`${window.location.origin}/api/clients/${clientId}/all-sessions${queryParams}`)
                .then(response => response.json())
                .then(allSessions => {
                    return fetch(`${window.location.origin}/api/clients/${clientId}/history${queryParams}`)
                        .then(response => response.json())
                        .then(tusHistory => ({ allSessions, tusHistory: tusHistory.tus_group || [] }));
                })
                .then(({ allSessions, tusHistory }) => {
                    // Tutaj wywołaj funkcję renderHistory
                    renderHistory(allSessions, tusHistory);
                })
                .catch(error => {
...
// Funkcja renderHistory musi być dostępna globalnie
window.renderHistory = function(allSessions, tusHistory) {
    // ... (taka sama implementacja jak wyżej)
    const individualAndJournalSessions = allSessions.filter(s => s.source_type !== 'tus');
...
// Pomocnicza funkcja do skracania tekstu
